...
import { ArrowLeft, CalendarClock, GraduationCap, MessageSquareText, User } from "lucide-react";
...
import type { TaskReview } from "@/store/task-default/task-default.type";
import {
...
function GradeDialog({ open, onOpenChange, review }: GradeDialogProps) {
    const [rate, rateState] = useRateTaskSubmissionMutation();
    const [unrate, unrateState] = useUnrateTaskSubmissionMutation();
    ...
    const onUnrate = async () => {
        ...
        try {
            await unrate({ id: review.id }).unwrap();
            onOpenChange(false);
            setGradeValue("");
            setComment("");
        } catch (_e) {}
        ...
        <Dialog open={open} onOpenChange={(v) => !isBusy && onOpenChange(v)}>
            <DialogContent className="sm:max-w-xl">
                <DialogHeader>
                    ...
                    <DialogDescription>
                        {review ? `Студент: ${safeName(review.student?.full_name ?? review.student?.email ?? "Немає")}` : "Немає"}
                    </DialogDescription>
                </DialogHeader>
                        ...
                        <CardContent className="pt-4 space-y-3">
                            ...
                            <div className="grid gap-2">
                                ...
                                <Textarea
                                    ...
                                    placeholder="Фідбек студенту (необовʼязково)"
                ...
                <DialogFooter className="gap-2 sm:gap-2">
                    <Button className="cursor-pointer" variant="outline" onClick={() => onOpenChange(false)} disabled={isBusy}>
                        Закрити
                    </Button>
                    ...
                    <Button
                        ...
                        variant="destructive"
                        onClick={onUnrate}
                        disabled={isBusy || !review || !(review as any)?.grade}
                    >
                        Зняти оцінку
                    </Button>
...
export default function TeacherTaskSubmissionsPage() {
    const router = useRouter();
    const params = useParams<{ taskId?: string; id?: string }>();

    const taskIdRaw = params?.taskId ?? params?.id ?? "";
    const taskId = Number(taskIdRaw);
    const canLoad = Number.isFinite(taskId) && taskId > 0;
    ...
    const { data, isLoading } = useGetTaskSubmissionReviewsQuery(
        canLoad
            ? {
                task_id: taskId,
                page,
        ...
        <div className="space-y-6">
            <div className="flex flex-col gap-3">
                ...
                <div className="flex flex-col md:flex-row md:items-end justify-between gap-3">
                    <div>
                        <h1 className="text-3xl font-bold">Здачі завдання</h1>
                        <p className="text-muted-foreground">{canLoad ? `Завдання номер: ${taskId}` : "Немає"}</p>
                    </div>
            ...
            <Card>
                ...
                <CardContent>
                    <ResponsiveList
                        ...
                        renderRow={(r: TaskReview, _open, onToggle) => {
                    ...
                    <span className="inline-flex items-center gap-2">
                      <CalendarClock className="h-4 w-4 text-muted-foreground" />
                        ...
                        renderMobileDetails={(r: TaskReview) => {
                            const studentName = safeName(r.student?.full_name ?? r.student?.email ?? "Немає");
                            const textPreview = (r.submission_text ?? "").trim();
                            const hasFiles = (r.files?.length ?? 0) > 0;
                            ...
                            return (
                                <div className="grid grid-cols-2 gap-x-4 gap-y-2 text-sm">
                                    ...
                                    <div className="text-right">{hasFiles ? `${r.files.length} файл(ів)` : "Немає"}</div>
