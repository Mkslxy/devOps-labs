...
export default function SchoolPickerDefault({ value, onChange }: Props) {
    ...
    const selectedSchool =
        value == null ? null : schools.find((s) => s.id === value) ?? null;
            ...
            {selectedSchool && (
                <Card className="p-4 bg-muted/30 space-y-2 gap-0">
                    ...
                    <div className="text-xs text-muted-foreground">
                        Місто: {selectedSchool.city || "Немає"}
                    </div>

                    <div className="text-xs text-muted-foreground">
                        Адреса: {selectedSchool.address || "Немає"}
                    </div>
                </Card>
            )}
        </div>
